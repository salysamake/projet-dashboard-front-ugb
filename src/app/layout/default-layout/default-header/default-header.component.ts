...
@Component({
  ...
  standalone: true,
  imports: [FilterComponent, ContainerComponent, HeaderTogglerDirective, SidebarToggleDirective, IconDirective, HeaderNavComponent, NavItemComponent, NavLinkDirective, RouterLink, RouterLinkActive, NgTemplateOutlet, BreadcrumbRouterComponent, ThemeDirective, DropdownComponent, DropdownToggleDirective, TextColorDirective, AvatarComponent, DropdownMenuDirective, DropdownHeaderDirective, DropdownItemDirective, BadgeComponent, DropdownDividerDirective, ProgressBarDirective, ProgressComponent, NgStyle]
})
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  seletedRegion : string = '';


  async onRegionRangeChanged(event: {region: string }): Promise<void> {
    this.seletedRegion = event.region;
    await this.applyFilters();
  }
  
  // Méthode asynchrone pour appliquer les filtres
  private async applyFilters(): Promise<void> {
     if (this.seletedRegion) {
     
  
      // Récupérer les données uniquement par application
      try {
        const data = await this.dataService.getDataByRegion(this.seletedRegion).toPromise();
        ...
        console.error('Erreur lors de la récupération des données par application', error);
      } 
    } else {
      alert('Please select both start and end dates or choose an application.');
    }
  }
